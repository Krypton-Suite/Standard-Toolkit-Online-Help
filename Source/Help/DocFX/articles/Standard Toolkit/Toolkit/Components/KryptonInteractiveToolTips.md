...
`KryptonToolTip` can show either the existing SuperTip (title, body, image) or an **interactive** popup that hosts any WinForms `Control` — for example a `KryptonLinkLabel` or a small panel with buttons. This implements [issue #4192](https://github.com/Krypton-Suite/Standard-Toolkit/issues/4192).
