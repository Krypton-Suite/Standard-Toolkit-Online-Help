...
To compile the source tree for yourself, run `run.cmd` from the repository root (Command Prompt or Windows Terminal). This launches an interactive menu for building Nightly, Canary, or Stable versions and creating NuGet packages. Alternatively, use `dotnet build "Source/Krypton Components/Krypton Toolkit Suite 2022 - VS2022.sln" -c Debug` for a quick Debug build, or `Scripts/VS2022/build-stable.cmd` for a Stable build. Script folders target different Visual Studio generations (`Scripts/VS2022/` → 2022, `Scripts/Current/` → 2026, `Scripts/Build/` → 2019); each locates MSBuild via `Scripts/Common/find-msbuild.cmd` and prints the resolved path and version at startup. See [Build Scripts](Build%20System/BuildScripts.md#msbuild-and-visual-studio-discovery). Orchestration scripts under `Scripts/` invoke MSBuild with `/m` (multi-processor) by default; see [Build Scripts](Build%20System/BuildScripts.md#parallel-builds). Outputs are stored in `Bin/<Configuration>/` (e.g., `Bin/Release`, `Bin/Nightly`, `Bin/Canary`); NuGet packages go to `Bin/Packages/<Configuration>/`.
