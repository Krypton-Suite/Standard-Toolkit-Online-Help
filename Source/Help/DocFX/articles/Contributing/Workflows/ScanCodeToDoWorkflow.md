...
This guide explains how the **Scan code TODOs** automation works, including **Code ToDo**, **HACK (workaround)**, and **FIXME → Bug Report** filing.

**NOTE:** Ensure that you give everyone on Discord advanced warning before a run, as it can open new issues which will notify Discord.
...
| `HACK(issue):` | Opt-in (default) | `[Code ToDo]: ...` + `has workaround` | `code_todo.yml` |
| `FIXME(issue):` | Opt-in (default) | `[Bug]: ...` | `bug_report.yml` |
| Plain `TODO` / `ToDo` | Only when `include_all_markers=true` | Code ToDo | `code_todo.yml` |
| Plain `HACK` | Only when `include_all_markers=true` | Code ToDo + `has workaround` | `code_todo.yml` |
...
For temporary workarounds that need a proper fix later.

```csharp
// HACK(issue): Bitmap fallback used because glyph path fails at high DPI
```

Uses the same **Code ToDo** body structure as `todo`, but:

- Adds the `has workaround` label
- Uses fingerprint `<!-- code-hack-scan:key=... -->`
- Notes in the body that the issue originated from a `HACK` comment
...
// TODO(issue): Description of work to track
// HACK(issue): Description of temporary workaround to replace
...
- Use **`HACK(issue):`** for temporary workarounds that need replacing
...
1. **Fingerprint** — `code-todo-scan`, `code-hack-scan`, or `code-fixme-scan` key in body
...
```
SHA-256("hack:" + normalizedText)   // when group_duplicates=true
SHA-256("fixme:" + normalizedText)
...
// HACK(issue):  -> Code ToDo issue + has workaround
...
| `markerType: hack` | Files a Code ToDo issue with `has workaround` |
| `markerType: fixme` | Files a Bug Report issue |
| `code-todo-scan` | Fingerprint namespace for Code ToDo issues |
| `code-hack-scan` | Fingerprint namespace for HACK workaround issues |
| `code-fixme-scan` | Fingerprint namespace for FIXME bug issues |

---

*Last updated for HACK (workaround) support.*
